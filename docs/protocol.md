...
The future Android client can demultiplex by stream ID, configure three MediaCodec H.264 decoders from the parameter sets, and submit each complete access unit with its timestamp to the corresponding decoder. Android/Quest decoding and display remain unimplemented and untested here.
